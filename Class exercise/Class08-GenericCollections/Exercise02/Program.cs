...
namespace Exercise02
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            Stefan.FavoriteSongs = Songs.Where(song => song.Genre == Genre.Hip_Hop && song.Length < 180).ToList();

            List<Person> FourOrMoreSongs = FansArray.Where(person => person.FavoriteSongs.Count >= 4).ToList();
            ...
            PersonsWitFourOrMoreSongs(FourOrMoreSongs);
        }

        private static void PersonsWitFourOrMoreSongs(List<Person> FourOrMoreSongs)
        {
            Console.WriteLine("Persons with 4 or more favorite songs:");
            foreach (var person in FourOrMoreSongs)
            {
